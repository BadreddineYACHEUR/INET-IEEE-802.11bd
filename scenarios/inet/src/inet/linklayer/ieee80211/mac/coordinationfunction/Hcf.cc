...
void Hcf::processUpperFrame(Ieee80211DataOrMgmtFrame* frame)
{
    ...
    if (edcaPendingQueues[ac]->insert(frame)) {
        EV_INFO << "Frame " << frame->getName() << " has been inserted into the PendingQueue. size = "<< edcaPendingQueues[ac]->getNumberOfFrames()<< endl;
...
void Hcf::channelGranted(IChannelAccess* channelAccess)
{
	...
	EV_DETAIL << "Start TXOP " << edcaTxops[ac]->getLimit() << std::endl;
...
void Hcf::transmitFrame(Ieee80211Frame* frame, simtime_t ifs)
{
    ...
    if (channelOwner) {
	    ...
	    EV_INFO << "Duration of the frame "<< frame->getDuration() << std::endl;}
