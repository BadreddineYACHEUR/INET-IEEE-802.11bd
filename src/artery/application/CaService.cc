...
/*
	...
	for (auto channel : mco.allChannels(aid::CA)) {
		auto network = networks.select(channel);
		if (network) {
			btp::DataRequestB req;
			// use same port number as configured for listening on this channel
			req.destination_port = btp::ports::CAM;
			req.gn.transport_type = geonet::TransportType::SHB;
			req.gn.its_aid = aid::CA;
			req.gn.maximum_lifetime = geonet::Lifetime { geonet::Lifetime::Base::One_Second, 1 };
...
vanetza::asn1::Cam createCooperativeAwarenessMessage(const VehicleDataProvider& vdp, uint16_t genDeltaTime)
{
	...
	basic.referencePosition.longitude = 0;
	basic.referencePosition.latitude = 0;
