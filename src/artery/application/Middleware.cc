...
ChannelNumber getChannelBD(const omnetpp::cXMLElement* cfg)
{
    ChannelNumber channel = -1;

    const char* ch_attr = cfg->getAttribute("channel-bd");
    if (ch_attr) {
        channel = parseChannelNumber(ch_attr);
    }
    ...
    for (cXMLElement* service_cfg : config->getChildrenByTagName("service")) {
        ...
        if (service_applicable) {
            ...
            if (service) {
                ...
                for (const cXMLElement* listener : service_cfg->getChildrenByTagName("listener")) {
                    if (listener->getAttribute("port")) {
                        ...
                        if (listener->getAttribute("channel")){
			...
			if (listener->getAttribute("channel-bd")){		//to consider a service operating using the 20MHz bd channel
                ...
                // promiscuous ITS-G5 services grab packets from CCH by default for mode (p) and SCH0_BD for mode (bd) if not specified otherwise
                if (promiscuous && channels == 0) {
                    mTransportDispatcher.addPromiscuousListener(promiscuous, channel::CCH);
		    mTransportDispatcher.addPromiscuousListener(promiscuous, channel::SCH0_BD);
                }
...
void Middleware::requestTransmission(const vanetza::btp::DataRequestB& request, std::unique_ptr<vanetza::DownPacket> packet)
{
    ...
    if (channels.empty()) {
        EV_WARN << "No channel found for ITS-AID Check the mco configuration in the omnetpp.ini file" << request.gn.its_aid << "\n";
    }
