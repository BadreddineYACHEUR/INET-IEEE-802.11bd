...
std::ostream& Ieee80211VHTMode::printToStream(std::ostream& stream, int level) const
{
    stream << "Ieee80211VHTMode";
    stream << ", netBitrate = " << getDataMode()->getNetBitrate();
	stream << ", mcsIndex = " << getDataMode()->getMcsIndex();
    return stream;
}
...
const simtime_t Ieee80211VHTMode::getSlotTime() const
{
    if (carrierFrequencyMode  == BAND_5GHZ)
		...
		if (carrierFrequencyMode == BAND_5_9GHZ_20MHZ)
			return 9E-6;
		else
