...
Define_Module(Radio);
simsignal_t Radio::receptionStateSignal = cComponent::registerSignal("receptionState");
...
simsignal_t Radio::transmissionDurationSignal = cComponent::registerSignal("transmissionDuration");
simsignal_t Radio::transmissionLengthSignal = cComponent::registerSignal("transmissionLength");
...
void Radio::startTransmission(cPacket *macFrame, IRadioSignal::SignalPart part)
{
    ...
    // EV_ERROR <<	"DURATION: " << transmission->getDuration() << "length" << macFrame->getByteLength() << endl;
    emit(transmissionDurationSignal, transmission->getDuration());
    emit(transmissionLengthSignal, macFrame->getByteLength());
    updateTransceiverState();
    updateTransceiverPart();
...
// TODO: this would draw twice from the random number generator in isReceptionSuccessful: auto isReceptionSuccessful = medium->isReceptionSuccessful(this, transmission, part);
	    ...
	    //std::cout << "isReceptionSuccessful" << isReceptionSuccessful;
	    emit(receptionStateSignal, isReceptionSuccessful);
