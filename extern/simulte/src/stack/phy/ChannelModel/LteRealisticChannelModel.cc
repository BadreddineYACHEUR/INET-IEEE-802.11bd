...
bool LteRealisticChannelModel::error_D2D(LteAirFrame *frame, UserControlInfo* lteInfo, const std::vector<double>& rsrpVector)
{
   ...
   // added by me
   // std::cout << "node: " << id << "getDirection D2D"<< (lteInfo->getDirection() == D2D) << " finalSuccess: " << finalSuccess << " totalPer: " << totalPer << " random error extracted " << er << " er <= totalPer " << (er <= totalPer) << "\n";
