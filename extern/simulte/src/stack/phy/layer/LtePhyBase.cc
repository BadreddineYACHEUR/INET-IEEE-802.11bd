...
simsignal_t LtePhyBase::transmissionDurationLTESignal = cComponent::registerSignal("transmissionDurationLTE");
simsignal_t LtePhyBase::transmissionLengthLTESignal = cComponent::registerSignal("transmissionLengthLTE");
...
void LtePhyBase::sendMulticast(LteAirFrame *frame)
{
    ...
    for (; nodeIt != binder_->getNodeIdListEnd(); ++nodeIt)
    {
	    ...
	    emit(transmissionDurationLTESignal, frame->getDuration());
   	    emit(transmissionLengthLTESignal, frame->getByteLength());
