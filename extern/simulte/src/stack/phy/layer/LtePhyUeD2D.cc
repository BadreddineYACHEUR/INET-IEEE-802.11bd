...
simsignal_t LtePhyUeD2D::numAirFrameReceivedSignal = cComponent::registerSignal("numAirFrameReceived");
simsignal_t LtePhyUeD2D::numAirFrameNotReceivedSignal = cComponent::registerSignal("numAirFrameNotReceived");
...
//    }
    ...
    UserControlInfo* lteInfo = check_and_cast<UserControlInfo*>(msg->removeControlInfo());
    // added by me
    std::cout << simTime() << " Direction " << ((lteInfo->getDirection() == D2D_MULTI) ? "D2D_MULTI" : "D2D");
    std::cout <<  "  " + std::to_string(nodeId_) << " lteInfo-> totalGranted= " << std::to_string(lteInfo->getTotalGrantedBlocks()) << "\n";
    ...
    EV << "LtePhyUeD2D::handleUpperMessage - " << nodeTypeToA(nodeType_) << " with id " << nodeId_
       << " sending message to the air channel. Dest=" << lteInfo->getDestId() << " size: " << frame->getByteLength() <<endl;
    ...
    if (lteInfo->getDirection() == D2D_MULTI){
        std::cout << simTime() ;
    	std::cout << "  " << std::to_string(nodeId_) << " duration = " << frame->getDuration() << " byteLength= " << std::to_string(frame->getByteLength()) << "\n";
...
void LtePhyUeD2D::storeAirFrame(LteAirFrame* newFrame)
{
    ...
    if (strcmp(par("d2dMulticastCaptureEffectFactor"),"RSRP") == 0)
    {
        ...
        EV << NOW << " LtePhyUeD2D::storeAirFrame - Average RSRP from node " << newInfo->getSourceId() << ": " << rsrpMean << " allocatedRbs: "<< allocatedRbs;
    }
    else  // distance
    {
...
//            m.setStart(das_->getAntennaCoord(*it));
    ...
    if (result){
        numAirFrameReceived_++;
        emit(numAirFrameReceivedSignal, numAirFrameReceived_);
    }else{
        numAirFrameNotReceived_++;
        emit(numAirFrameNotReceivedSignal, numAirFrameNotReceived_);
    }
    EV << "Handled LteAirframe with ID " << frame->getId() << " with result "
       << ( result ? "RECEIVED" : "NOT RECEIVED" ) << endl;
